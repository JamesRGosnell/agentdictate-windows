...
fn history_off_keeps_usage_numbers_but_no_transcript_after_delivery() {
    ...
    drop(connection);
    drop(runtime);
    // Enabling history later must not restore a transcript recorded while off.
    let mut restarted = Runtime::open(&database_path).unwrap();
    restarted
        .clean_up_finished_jobs(&Settings::default(), &directory.path().join("recordings"))
        .unwrap();
    assert!(
        restarted
            .list_history(HistoryQuery::all())
            .unwrap()
            .is_empty()
    );
    assert_eq!(
        restarted.usage_summary().unwrap().all_time.total_sessions,
        1
    );
}

#[test]
fn clearing_history_suppresses_a_delivery_waiting_for_background_completion() {
    let directory = TempDir::new().unwrap();
    let database_path = directory.path().join("agentdictate.db");
    let mut runtime = Runtime::open(&database_path).unwrap();
    let delivered = delivered_job(&mut runtime, &directory);
    let mut worker = Runtime::open_background_writer(&database_path).unwrap();
...
#[test]
fn crash_recovery_does_not_enable_history_for_a_recording_started_with_history_off() {
    let directory = TempDir::new().unwrap();
    let database_path = directory.path().join("agentdictate.db");
    let mut runtime = Runtime::open(&database_path).unwrap();
    let settings = Settings {
    ...
    let mut request = request(
        &directory.path().join("history-off.wav"),
        TRANSCRIPTION_MODEL,
    );
    request.options = Some(agentdictate_core::DictationOptions::from_settings(
        &settings,
        vec![],
    ));
    let job = runtime
    ...
    // The paste succeeded, but bookkeeping did not run before a restart.
    let mut restarted = Runtime::open(&database_path).unwrap();
    restarted
...
#[test]
fn completion_failure_rolls_back_history_and_preserves_the_delivered_checkpoint() {
    let directory = TempDir::new().unwrap();
    let database_path = directory.path().join("agentdictate.db");
    let mut runtime = Runtime::open(&database_path).unwrap();
    let delivered = delivered_job(&mut runtime, &directory);
    ...
    connection
        .execute_batch(
            "CREATE TRIGGER reject_completion BEFORE DELETE ON dictation_jobs
    ...
    assert_eq!(
        runtime.job(delivered.id).unwrap().unwrap().stage,
        JobStage::Delivered
    );
    assert!(
        runtime
    ...
    restarted
        ...
        .unwrap();
    assert_eq!(
        restarted.list_history(HistoryQuery::all()).unwrap().len(),
        1
    );
    assert_eq!(
        restarted.usage_summary().unwrap().all_time.total_sessions,
    ...
    assert!(restarted.job(delivered.id).unwrap().is_none());
}
