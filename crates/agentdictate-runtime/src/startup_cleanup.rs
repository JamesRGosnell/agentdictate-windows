...
impl Runtime {
    ...
    /// latter only when `save_history` permits it) exactly once. Legacy finished
    /// rows never recreate missing History or usage. Audio of finished
    ...
    fn remove_orphan_recordings(
        ...
        let owned = {
            ...
            statement
                ...
                .collect::<rusqlite::Result<Vec<_>>>()?
                .into_iter()
                .filter_map(|path| Path::new(&path).file_name().map(recording_file_key))
        ...
        for entry in entries {
            ...
            if path.extension() != Some(OsStr::new("wav"))
                || owned.contains(&recording_file_key(&entry.file_name()))
            {
                continue;
            }
...
fn recording_file_key(name: &OsStr) -> OsString {
    // Generated recording names are ASCII. Windows path casing must never
    // make an owned recovery file look like an orphan.
    #[cfg(windows)]
    {
