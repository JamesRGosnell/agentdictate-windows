...
impl Runtime {
    ...
    pub(crate) fn complete_delivered_job(
        ...
        let already_recorded = transaction
            ...
            .optional()?
            .is_some();
        let recording_allowed: bool = transaction.query_row(
            "SELECT completion_recording_allowed FROM dictation_jobs WHERE runtime_id = ?1",
            [job_id.to_string()],
            |row| row.get(0),
        )?;
        let recorded = !already_recorded && recording_allowed;
        if recorded {
    ...
    pub fn clear_history(&mut self) -> Result<(), RuntimeError> {
        let transaction = self.connection.transaction()?;
        // Maintenance can finish a delivered job on another connection.
        // Clearing history must also suppress those pending completions.
        transaction.execute(
...
fn record_session(
    ...
    let session_id = transaction.last_insert_rowid();
    let saved_history_preference = job
        .options
        .as_ref()
        .and_then(|options| options.save_history);
    if settings.save_history && saved_history_preference.unwrap_or(true) {
        transaction.execute(
            r#"
            INSERT INTO transcript_history (
