...
fn client_commands_have_a_versioned_stable_wire_shape() {
    ...
    assert_eq!(
        wire,
        r#"{"protocol_version":6,"command":"start_recording","request_id":7}"#
    );
}
...
fn rejected_commands_return_a_correlated_error_instead_of_looking_successful() {
    ...
    assert_eq!(
        ...
        r#"{"protocol_version":6,"message":"command_rejected","request_id":19,"error":"microphone unavailable"}"#
    );
}
...
fn history_page_requests_are_bounded_and_typed_on_the_wire() {
    ...
    assert_eq!(
        wire,
        r#"{"protocol_version":6,"command":"get_history_page","request_id":12,"request":{"search":"database migration","page_size":20,"after":"opaque-page-2"}}"#
    );
    assert_eq!(
...
fn workspace_mutations_are_typed_and_round_trip() {
    ...
    let commands = [
        ...
        ClientCommand::copy_recovery(26, agentdictate_core::JobId::new()),
    ];
